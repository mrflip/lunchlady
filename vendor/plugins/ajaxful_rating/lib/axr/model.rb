module AjaxfulRating # :nodoc:
  ...
  module InstanceMethods
    ...
    def wrapper_dom_id(options = {})
      ...
      parts = options.slice(:size, :dimension).select{|k, v| v.present? or (v == false) }.map do |k, v|
        if k == :dimension
          v.to_s
        else
      ...
      if options[:show_user_rating]
        parts << "user_#{options[:current_user].id}"
      end
      parts.unshift("ajaxful_rating")
      ApplicationController.helpers.dom_id(self, parts.sort.join('_'))
    end
